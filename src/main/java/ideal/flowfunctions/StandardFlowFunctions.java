	...
	public FlowFunction<UpdatableAccessGraph> getNormalFlowFunction(final UpdatableAccessGraph sourceFact, final UpdatableWrapper<Unit> curr,
		...
		return new FlowFunction<UpdatableAccessGraph>() {
			...
			public Set<UpdatableAccessGraph> computeTargets(UpdatableAccessGraph source) {
				IExtendedICFG<Unit, SootMethod> icfg = context.icfg();
				...
				if (AliasFinder.HANDLE_EXCEPTION_FLOW && !source.isStatic() && curr.getContents() instanceof IdentityStmt) {
					...
					if (identityStmt.getRightOp() instanceof CaughtExceptionRef
						...
						if (!source.isStatic()){
							...
							out.add(source);
							out.add(source.deriveWithNewLocal(icfg.wrap((Local) leftOp)));
							return out;
						} else{
							return Collections.emptySet();
						}
				...
				if (!(curr.getContents() instanceof AssignStmt)) {
					if (curr.getContents() instanceof IfStmt) {
						...
						if (condition instanceof EqExpr && source.getFieldCount() == 0) {
							...
							if (rightOp instanceof NullConstant && leftOp.equals(source.getBase().getContents())) {
								nullnessCheck = true;
							} else if (leftOp instanceof NullConstant && rightOp.equals(source.getBase().getContents())) {
								nullnessCheck = true;
							}
				...
				if (rightOp instanceof Constant || rightOp instanceof NewExpr) {
					// a = new || a = 2
					if (leftOp instanceof Local && source.baseMatches(icfg.wrap(leftOp)))
						// source == a.*
						return Collections.emptySet();
					// a.f = new || a.f = 2;
					if (leftOp instanceof InstanceFieldRef) {
						...
						// source == a.f.*
						if (source.baseAndFirstFieldMatches(icfg.wrap(base), icfg.wrap(field)))
							return Collections.emptySet();
					}
				...
				if (leftOp instanceof Local) {
					if (source.baseMatches(icfg.wrap(leftOp))) {
				...
				if (rightOp instanceof CastExpr) {
					...
					if (op instanceof Local) {
						if (!source.isStatic() && source.baseMatches(icfg.wrap(op))) {
							out.add(source.deriveWithNewLocal(icfg.wrap((Local) leftOp)));
						}
					}
				}

				if (rightOp instanceof Local && source.baseMatches(icfg.wrap(rightOp))) {
					...
					if (source.baseMatches(icfg.wrap((Value) base)) && source.firstFirstFieldMayMatch(icfg.wrap(field))) {
						// e = a.f && source == a.f.*
						// replace in source
						if (leftOp instanceof Local && !source.baseMatches(icfg.wrap(leftOp))) {
							for(UpdatableWrappedSootField firstField : source.getFirstField()){
								UpdatableAccessGraph deriveWithNewLocal = source.deriveWithNewLocal(icfg.wrap((Local) leftOp));
			...
			public Set<UpdatableAccessGraph> computeTargets(UpdatableAccessGraph source) {
				...
				if (is.containsInvokeExpr()) {
					...
					for (int i = 0; i < paramLocals.length; i++) {
						...
						if (arg instanceof Local && source.baseMatches(icfg.wrap(arg))) {
							if (!pointsToSetEmpty(paramLocals[i])) {
								out.add(source.deriveWithNewLocal(icfg.wrap(paramLocals[i])));
							}
						}
					}
					...
					if (!callee.getContents().isStatic() && ie instanceof InstanceInvokeExpr) {
						...
						if (source.baseMatches(icfg.wrap(iIExpr.getBase()))) {
							...
							if(!pointsToSetEmpty(thisLocal)){
								UpdatableAccessGraph replacedThisValue = source.deriveWithNewLocal(icfg.wrap(thisLocal));
		...
		return new FlowFunction<UpdatableAccessGraph>() {
			...
			public Set<UpdatableAccessGraph> computeTargets(UpdatableAccessGraph source) {
				...
				if (callSite.getContents() instanceof Stmt) {
					...
					if (is.containsInvokeExpr()) {
						...
						for (int i = 0; i < paramLocals.length; i++) {
							...
							if (paramLocals[i].equals(source.getBase().getContents())) {
								Value arg = ie.getArg(i);
								if (arg instanceof Local) {
									if(pointsToSetEmpty((Local) arg))
										return Collections.emptySet();
									UpdatableAccessGraph deriveWithNewLocal = source.deriveWithNewLocal(icfg.wrap((Local) arg));
						...
						if (!callee.getContents().isStatic() && ie instanceof InstanceInvokeExpr) {
							if (source.baseMatches(icfg.wrap((Value) thisLocal))) {

								InstanceInvokeExpr iIExpr = (InstanceInvokeExpr) is.getInvokeExpr();
								Local newBase = (Local) iIExpr.getBase();
								if(pointsToSetEmpty((Local) newBase))
									return Collections.emptySet();
								if (pointsToSetCompatible(newBase, source.getBase().getContents())) {
									UpdatableAccessGraph possibleAccessPath = source.deriveWithNewLocal(icfg.wrap((Local) iIExpr.getBase()));
				...
				if (callSite.getContents() instanceof AssignStmt && exitStmt.getContents() instanceof ReturnStmt) {
					...
					// d = return out;
					if (leftOp instanceof Local) {
						if (returns instanceof Local && source.getBase().getContents().equals(returns) && !pointsToSetEmpty((Local)leftOp)) {
							out.add(source.deriveWithNewLocal(icfg.wrap((Local) leftOp)));
						}
					}
				}
			...
			public Set<UpdatableAccessGraph> computeTargets(UpdatableAccessGraph source) {
				...
				if(hasCallees){
					for (int i = 0; i < invokeExpr.getArgCount(); i++) {
						if (source.baseMatches(context.icfg().wrap(invokeExpr.getArg(i)))) {
							return  Sets.newHashSet();
						}
					}
					if (invokeExpr instanceof InstanceInvokeExpr) {
						...
						if (source.baseMatches(context.icfg().wrap(base))) {
							return  Sets.newHashSet();
						}
					}
				}
